...
from tests.fixtures.flight_ads import flight_11158378, \
    flight_11322212, \
...
class TestHandler(TestCase):
    ...
    def test_get_all_active_creative_maps(self):
        ...
        responses.add(responses.GET, handler.ADZERK_FLIGHT.format('11322212'), json=flight_11322212)
        # 11158378 has 3 creative maps and 1 site/zone targeting = 3 ad objects
        responses.add(responses.GET, handler.ADZERK_FLIGHT.format('11158378'), json=flight_11158378)
        # 8000 has no creatives, 0 ad objects
        responses.add(responses.GET, handler.ADZERK_FLIGHT.format('8000'), json=flight_8000_active_no_creatives)
        # 8001 has 3 creatives, 1 zone targeting, 3 ad objects
        responses.add(responses.GET, handler.ADZERK_FLIGHT.format('8001'), json=flight_8001_active_no_site_targeting)
        # 8002 has 3 creatives, 1 site targeting, 3 ad objects
        responses.add(responses.GET, handler.ADZERK_FLIGHT.format('8002'), json=flight_8002_active_no_zone_targeting)
        # 8003 has 3 creatives, no site/zone targeting, 3 ad objects
        responses.add(responses.GET, handler.ADZERK_FLIGHT.format('8003'), json=flight_8003_active_no_targeting)
        # 8004 has 1 creative with a non-standard template, 1 site/zone targeting, 1 ad object
        responses.add(responses.GET, handler.ADZERK_FLIGHT.format('8004'), json=flight_8004_active_non_standard_creative_template)
        # expect 19 objects
        ...
        for ad in ad_map:
            ...
            for key,value in ad.items():
                if ad['flight_id'] == 8001 and key == 'site_id':
                    ...
                    self.assertIsNone(value)
                elif ad['flight_id'] == 8004 and (key == 'sponsor' or key == 'creative_title' or key == 'creative_url'):
                    self.assertIsNone(value)
                else:
    ...
    def test_decorate_with_info_using_ad_value(self):
        ...
        actual_ads = handler.decorate_with_info(ads_, campaign_name_map, 'campaign_name', 'campaign_id')
        for i in range(5):
    ...
    def test_decorate_with_info_using_named_key(self):
        ...
        for i in range(5):
            ...
            modified_ad = handler.decorate_with_info([ads_[i]], rates, 'rate_type', i, ad_value=False)
            self.assertEqual(rates[i], modified_ad[0]['rate_type'])

    @patch('app.handler.__get_api_key', return_value=None)
    @responses.activate
    def test_main_handler_ads(self, mock_key):
        # set up responses
        responses.add(responses.GET, handler.ADZERK_ALL_FLIGHTS, json=full_test.all_flights_1)
        responses.add(responses.GET, handler.ADZERK_FLIGHT.format(str(8144015)), json=full_test.flight_1)
        responses.add(responses.GET, handler.ADZERK_ALL_CAMPAIGNS, json=full_test.campaign_1)
        ...
        ads = handler.main_handler(None, None, testing=True)
        ads_simple = ads['body']['ads']
        ...
        for ad_type in ad_nests:
            for ad in ad_type:
                if ad['site_id'] == 1082659:
                    ...
                    raise AssertionError('Site id to name mapping is wrong for ad id {}'
                                         .format(ad['ad_id']))
                ...
                if ad['zone_id'] == 204604:
                    ...
                    raise AssertionError('Zone id to name mapping is wrong for ad id {}'
                                         .format(ad['ad_id']))

                if ad['campaign_id'] == 838705:
                    self.assertEqual('My Test Campaign', ad['campaign_name'])
                else:
                    raise AssertionError('campaign id to name mapping is wrong for ad id {}'
                                         .format(ad['ad_id']))
