...
def test_big_query_handler_run_0(run_bigquery, event):
    ...
    assert len(bq.tag_records) == 1
    assert len(bq.run_records) == 1
    
    tags = [t for t, v in asdict(bq.tag_records[0]).items() if v is not None]
    # Add 2 to account for non tag fields "task_id" and "submission_date".
    assert len(event["task"]["tags"]) + 2 == len(tags)
