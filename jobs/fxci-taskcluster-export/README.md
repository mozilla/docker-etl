...
* `FXCI_ETL_BIGQUERY_CREDENTIALS` (str): Base64 encoded contents of a service account credentials file (required).
    The credentials require the DataWriter role for the configured dataset.
...
* `FXCI_ETL_STROAGE_CREDENTIALS` (str): Base64 encoded contents of a service account credentials file (required).
    The credentials require Read/Write access to the configured bucket.
...
* `FXCI_ETL_MONITORING_CREDENTIALS` (str): Base64 encoded contents of a service account credentials file (required).
    The credentials require the `MonitoringViewer` on the Firefox-CI worker projects (defined in config.py).
