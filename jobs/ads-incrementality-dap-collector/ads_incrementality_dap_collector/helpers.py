...
def collect_dap_result(task_id: str, vdaf_length: int, hpke_token: str, hpke_config: str, hpke_private_key: str, batch_start: int, duration: int) -> dict:
    ...
    print(f"Processing batch_start: {batch_start} for duration: {duration}")
    try:
        ...
        print(f"Collection failed for {task_id}, {e.returncode}, stderr: {e.stderr}")
    except subprocess.TimeoutExpired as e:
        # logging.error(f"Collection timed out for {task_id}, {e.timeout}, stderr: {e.stderr}")
        print(f"Collection timed out for {task_id}, {e.returncode}, stderr: {e.stderr}")
...
def correct_wraparound(num: int) -> int:
    ...
    if num > cutoff:
        ...
        print(f"Corrected {num} to {num - field_prime} ")
