    ...
    try:
        ...
        for line in result.stdout.splitlines():
            if line.startswith("Aggregation result:"):
                ...
                return entries
    # Beware! Exceptions thrown by the subprocess reveal secrets. Log them and include traceback only for debugging in local dev.
    except subprocess.CalledProcessError as e:
        raise Exception(f"Collection failed for {task_id}, {e.returncode}, stderr: {e.stderr}") from None
