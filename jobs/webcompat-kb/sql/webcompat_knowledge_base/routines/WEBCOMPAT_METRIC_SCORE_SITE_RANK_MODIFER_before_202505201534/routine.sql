CREATE OR REPLACE FUNCTION `{{ ref(name) }}`(url STRING, crux_yyyymm INT64) RETURNS FLOAT64 AS (
(SELECT
     IFNULL(IF(MIN(host_ranks.global_rank) <= MIN(host_ranks.local_rank), MAX(1.5 * global_scores.score), MAX(local_scores.score)), 1)
   FROM
