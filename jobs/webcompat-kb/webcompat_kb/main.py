...
class BugzillaToBigQuery:
    ...
    def run(self):
        ...
        dep_ids = set(item for sublist in kb_dep_ids.values() for item in sublist)

        # Check for missing bugs
        all_ids = set([item["id"] for item in all_bugs])
        ...
        self.record_import_run(
            self.history_fetch_completed, len(all_bugs), len(history_changes)
        )
