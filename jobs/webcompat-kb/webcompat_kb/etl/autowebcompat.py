...
class ReproTask(HackbotTask):
    ...
    user_story_fields = (
        "autowebcompat-repro-status",
        "autowebcompat-repro-reason",
        "autowebcompat-repro-chrome-mask-fixed",
        "autowebcompat-repro-channels",
        "autowebcompat-repro-report-os",
    )
    ...
    def populate_updates(self, updater: Updater) -> None:
        if isinstance(updater, BugzillaUpdater):
            ...
            for uuid, output in run_outputs.items():
                ...
                require_whiteboard = []
                require_user_story: dict[str, Optional[str]] = {
                    item: None for item in self.user_story_fields
                }

                bug_update.remove_whiteboard(
                    [
                        self.whiteboard_progress_token,
                        "[autowebcompat:repro-failed]",
                        "[autowebcompat:repro-success]",
                    ],
                )
...
class DiagnosisTask(HackbotTask):
    ...
    user_story_fields = (
        "autowebcompat-diagnosis-status",
        "autowebcompat-diagnosis-reason",
    )
