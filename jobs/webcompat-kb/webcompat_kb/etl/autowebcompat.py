...
def remove_user_story_fields(
    bug: bugzilla.Bug, bug_update: BugUpdate, remove_fields: list[str]
) -> None:
    current_user_story = (
        bug_update.bug.cf_user_story
        if bug_update.bug.cf_user_story is not None
        ...
        if key in current_fields:
            ...
            if isinstance(current_value, list):
                for value in current_value:
                    changes.append(userstory.UserStoryChange.delete(key, value))
            else:
            ...
            for uuid, (bug_number, output) in run_outputs.items():
                ...
                if isinstance(output, ErrorSummary):
                    ...
                    if result is None:
                        ...
                        remove_user_story_fields(
                            bug, bug_update, ["autowebcompat-diagnosis-reason"]
                        )
