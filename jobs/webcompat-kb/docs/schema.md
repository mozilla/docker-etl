...
The metadata is in `meta.toml` and the schema defintion is in `table.toml`.

In addition to the standard metadata, tables must have the following metadata:

* `etl` - A list of the etl jobs that will populate the table, if any.

They may also have the following metadata:

* `partition` - For tables that are partitioned, the partition definition.

Currently only integer range partitions are supported. They are defined in the following format:
...
So for example a table partitioned on the `"bucket"` key from 1-16 might have the meta.toml

```
name = "example_table"
description = "A table partitioed on an integer key"
etl = ["job-name"]
...
`RECORD` type fields need to have the schema for their sub-fields
defined. These use the format:

```toml
[field_name]
type = "RECORD"

[field_name.fields.sub_field]
type = <type>
mode = <mode>
```

i.e. we define a property `fields` on the `field_name` object which
has a key for each entry whose value has the same format as for the
main field.
