...
def main(project_id, submission_date, dry_run, backfill, start_date, end_date):
    assert submission_date is not None or backfill, "Either a submission date or the --backfill flag is required"
    
    if backfill:
        assert start_date is not None and end_date is not None, "You must provide a start and end date to backfill"
        if dry_run:
            print(f"Dry-run mode: Skipping download of {TICKER_LIST} from {start_date} to {end_date}")
        else:
            macro_df = get_macro_data(start_date, end_date)
    
    else:
        if dry_run:
            print(f"Dry-run mode: Skipping download of {TICKER_LIST} from {submission_date}")
        else:
            macro_df = get_macro_data(submission_date, submission_date)
    
    if dry_run:
        print(f"Dry-run mode: Skipping loading data to `{project_id}.{DATASET}.{TABLE}`")
        return
