...
parser.add_argument(
    ...
    help="Project containing views to be exported",
)
parser.add_argument(
    "--destination_project",
    "--destination-project",
...
def view_to_tmp_table(view, tmp_dataset, source_project):
    ...
    tmp_table = f"{source_project}.{tmp_dataset}.{tmp_table_name}"

    job_config = bigquery.QueryJobConfig(destination=tmp_table)
    job_config.clustering_fields = ["experiment"]
...
def export_dataset(
    ...
    # usually datasets to be exported are views
    # exporting to GCS only works for tables, so write results to a temporary table
    tmp_table = view_to_tmp_table(dataset, "tmp", source_project)
    ...
    client.delete_table(tmp_table, not_found_ok=True)  # remove tmp table
