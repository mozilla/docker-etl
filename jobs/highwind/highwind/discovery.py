...
# few days, so the tail costs the most cells and carries the least data. Capping the horizon says
# the interesting result is a unit's first six months, which is also what the analysis windows
# people read are.
MAX_WINDOW_DAYS = 180
...
def discover(client, as_of, limit=None, only_slugs=None):
    ...
    A recipe whose (app, randomization unit) pair has no analysis unit is skipped for the same
    reason. Both refusals are reported, so the run's count still accounts for every recipe read.
    """
