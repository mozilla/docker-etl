...
class TestCreateNewJob(TestCase):
    @classmethod
    def template_directory(cls):
        return os.path.join(os.path.dirname(__file__), "test_templates")

    @classmethod
    def default_job_directory(cls):
        return os.path.join(cls.template_directory(), "default")

    def test_copy_template_success(self):
        job_name = "_test_job_1"

        with tempfile.TemporaryDirectory() as temp_dir:
            with patch("docker_etl.create_new_job.JOBS_DIR", temp_dir):
